...
export function spawnPickup(kind, data, angleHint){
  ...
  if (kind === 'item'){
    const it = item(data);
    const base = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.45, 0.5), lam(0x8a6a3a));
    const lid  = new THREE.Mesh(new THREE.BoxGeometry(0.74, 0.18, 0.54), lam(it.color, 0.35)); lid.position.y = 0.32;
    const clasp = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.14, 0.06), lam(0xc9a15a)); clasp.position.set(0, 0.22, 0.28);
    g.add(base, lid, clasp, pointLight(it.color, 0.9, 6));
    g.userData = {kind, id:data};
  } else {
    const tok = new THREE.Mesh(new THREE.IcosahedronGeometry(0.42, 0), lam(0xffe9b0, 0.9));
