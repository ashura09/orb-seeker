...
export function markExplored(dt){
  ...
  const cx = cellOf(player.position.x), cz = cellOf(player.position.z);
...
export function forgetExplored(){ grid.fill(0); }
...
function buildBase(){
  ...
  for (let py = 0; py < S; py++){
    for (let px = 0; px < S; px++){
      ...
      if (Math.hypot(x, z) > WORLD_R){        // beyond where you can walk
        img.data[i] = 24; img.data[i+1] = 30; img.data[i+2] = 26; img.data[i+3] = 255;
        continue;
      }
      ...
      if (isInWater(x, z)){
        ...
        const c1 = region.ground[0], c2 = neighbour.ground[0];
      ...
      const e = SPAN / S;
      const dh = (heightAt(x + e, z) - heightAt(x - e, z))
