...
function rememberCamDist(){
  ...
  saveTimer = setTimeout(() => { save.camDist = G.camDist; persist(); }, 250);
}
...
function commitPinch(gap){
  pinch.active = true; pinch.candidate = false;
  releaseJoystick();
  look.active = false;
  ...
  pinch.gap = gap; pinch.dist0 = G.camDist;
}

function endPinch(){
  const wasActive = pinch.active;
  pinch.active = pinch.candidate = false; pinch.a = pinch.b = null;
  ...
  if (wasActive && rest.length === 1){
    ...
    look.active = true; look.id = rest[0]; look.lastX = t.x; look.lastY = t.y;
  }
}
...
function moveTouch(tch){
  ...
  if (t){ t.x = tch.clientX; t.y = tch.clientY; }

  // Zoom first: once a pinch is running it owns both fingers, so walking and
  // looking must not also act on them.
  if ((pinch.active || pinch.candidate) && (tch.identifier === pinch.a || tch.identifier === pinch.b)){
    const A = touches.get(pinch.a), B = touches.get(pinch.b);
    if (A && B){
      const gap = gapOf(A, B);
      if (pinch.candidate){
        const C = CONFIG.camera;
        if (Math.abs(gap - pinch.gap0) > C.pinchThreshold && joyStray() < C.pinchJoyTolerance) commitPinch(gap);
      }
      if (pinch.active){
...
function endTouch(tch){
  ...
  if ((pinch.active || pinch.candidate) && (tch.identifier === pinch.a || tch.identifier === pinch.b)) endPinch();
